            ...
            fs.stat(file, function(err, stat) {
                ...
                if (stat && stat.isDirectory()) {
                    if (options.recursive) {
                        if (options.matchDir && !matches(file, options.matchDir)) return next();
                        if (options.excludeDir && matches(file, options.excludeDir)) return next();
                    ...
                    if (options.match && !matches(file, options.match)) return next();
                    if (options.exclude && matches(file, options.exclude)) return next();
                    if (options.filter && !options.filter(file)) return next();
                    if (options.shortName) files.push(file);
